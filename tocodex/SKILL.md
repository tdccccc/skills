...
- Keep all temporary files under `docs/tocodex/{task_id}/`.
...
- Do not write temporary files outside `docs/tocodex/{task_id}/`.
- Preserve unrelated user changes.
...
All task artifacts live under `docs/tocodex/<task-id>/`. There is no separate `.codex-runs/` directory — stdout and stderr logs are written alongside the task file and report.
...
codex -a never exec \
  ...
  [--search] \
  [-i <image-path>] \
  '<canonical prompt>' </dev/null 2>docs/tocodex/<task-id>/stderr.log \
  | tee docs/tocodex/<task-id>/stdout.log
```
...
```
<task>Execute docs/tocodex/<task-id>/task.md in <target-project>. Follow the task contract exactly, do not stage or commit, and write the report to the specified path.</task>
```
...
- No `git add`
- No `git commit`
- All temporary files must live under `docs/tocodex/<task-id>/`
