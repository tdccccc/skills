...
- Keep all temporary files under `docs/tocodex-task/{task_id}/`.
...
```text
docs/tocodex-task/{task_id}/codex-report.md
```
...
- Do not write temporary files outside `docs/tocodex-task/{task_id}/`.
- Preserve unrelated user changes.
...
All task artifacts live under `docs/tocodex-task/<task-id>/`. There is no separate `.codex-runs/` directory — stdout and stderr logs are written alongside the task file and report.
...
codex -a never exec \
  ...
  [--search] \
  [-i <image-path>] \
  '<canonical prompt>' </dev/null 2>docs/tocodex-task/<task-id>/stderr.log \
  | tee docs/tocodex-task/<task-id>/stdout.log
```
...
```
<task>Execute docs/tocodex-task/<task-id>/task.md in <target-project>. Follow the task contract exactly, do not stage or commit, and write the report to the specified path.</task>
```

### Agent's responsibilities

1. Change to the target project directory
2. Create `docs/tocodex-task/<task-id>/` directory (if not already created by task.md)
...
1. Create a new `docs/tocodex-task/<new-task-id>/task.md`
...
- No `git add`
- No `git commit`
- All temporary files must live under `docs/tocodex-task/<task-id>/`
